...
//     IronSource.init(YOUR_APP_KEY, ['INTERSTITIAL']);
...
const ShowAdScreen = () => {
  useEffect(() => {
    ...
    IronSource.init('1ef4f198d');
  }, []);

  const showInterstitialAd = () => {
    IronSource.isInterstitialReady().then(isReady => {
      if (isReady) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.text}>Show Interstitial Ad Example</Text>
      <Button title="Show Ad" onPress={showInterstitialAd} />
    </View>
  );
};
