...
/**
 * 광고 식별자(AdID/IDFA) 가져오기
 * @returns {Promise<string>} 광고 식별자
 */
async function getAdvertisingId() {
  try {
    console.log('=== 광고 식별자 가져오기 시작 ===');

    // 1. react-native-google-mobile-ads를 사용하여 정확한 광고 식별자 가져오기
    const adId = await MobileAds.getAdId();
    console.log('MobileAds.getAdId() 결과:', adId);

    if (adId && adId.length === 36 && adId.includes('-')) {
      console.log('✅ 올바른 UUID 형식의 광고 식별자:', adId);
      return adId;
    }

    // 2. Android의 경우 Google Play Services Advertising ID 직접 확인
    if (Platform.OS === 'android') {
      try {
        const androidId = await DeviceInfo.getAndroidId();
        console.log('DeviceInfo.getAndroidId() 결과:', androidId);

        // Android ID가 UUID 형식인지 확인
        if (androidId && androidId.length === 16) {
          console.log('⚠️ Android ID 형식 (16자리):', androidId);
          console.log(
            '이는 Google Play Services Advertising ID가 아닐 수 있습니다.',
          );
        }
      } catch (error) {
        console.log('Android ID 가져오기 실패:', error);
      }
    }
    ...
    if (Platform.OS === 'ios') {
      try {
        ...
        console.log('DeviceInfo.getUniqueId() 결과:', uniqueId);
      } catch (error) {
        console.log('iOS Unique ID 가져오기 실패:', error);
      }
    }

    // 4. 대체값 사용
    console.warn('⚠️ 광고 식별자를 가져올 수 없어 디바이스 ID 사용');
    const deviceId = await DeviceInfo.getUniqueId();
    console.log('대체 디바이스 ID:', deviceId);
    return deviceId;
  } catch (error) {
    console.error('❌ 광고 식별자 가져오기 실패:', error);
    const deviceId = await DeviceInfo.getUniqueId();
    console.log('에러 시 대체 디바이스 ID:', deviceId);
    return deviceId;
  }
}
